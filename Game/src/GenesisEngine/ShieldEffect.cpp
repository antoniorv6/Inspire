 ...
 * @param l_shader_ptr 
 */
ShieldEffect::ShieldEffect(uint32_t l_screenWidth, uint32_t l_screenHeight, TResourceShader * l_shader_ptr, const std::string& l_ID) : IEffect(l_shader_ptr, l_ID)
{
    m_renderUtilities_ptr = RenderUtilities::getInstance();
    m_blendingTexture = (TResourceTexture*)(ResourceManager::getInstance()->GetTexture("shield_hit.png"));
 ...
 * @param l_screenHeight 
 */
void ShieldEffect::Init(uint32_t l_screenWidth, uint32_t l_screenHeight)
{
    ...
    {
        //std::cout << "ERROR::SHIELD::FRAMEBUFFER:: Framebuffer is not complete!" << std::endl;
    }
    else
    {
        //std::cout << "SUCCESS::SHIELD::FRAMEBUFFER:: Framebuffer complete!" << std::endl;
    }
 ...
 * @param dt 
 */
void ShieldEffect::Draw(float dt)
{
    m_renderUtilities_ptr->UpdateShieldValues();
    
    if(!m_renderUtilities_ptr->IsShieldDesactivated())
    {
        /**
        ...
        m_shader_ptr->UseShader();
        m_shader_ptr->SetFloat("_shieldAlpha", m_renderUtilities_ptr->GetShieldAlpha());
        m_shader_ptr->SetFloat("_redPct", m_renderUtilities_ptr->GetRedPct());
        m_shader_ptr->SetFloat("_bluePct", m_renderUtilities_ptr->GetBluePct());
    ...
    {
        m_finished = true;
        m_renderUtilities_ptr->ResetShieldValues();
    }
