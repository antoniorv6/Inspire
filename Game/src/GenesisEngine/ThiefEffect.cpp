 ...
 * @param l_shader_ptr 
 */
ThiefEffect::ThiefEffect(uint32_t l_screenWidth, uint32_t l_screenHeight, TResourceShader * l_shader_ptr, const std::string& l_ID) : IEffect(l_shader_ptr, l_ID)
{
 ...
 * @param l_screenHeight 
 */
void ThiefEffect::Init(uint32_t l_screenWidth, uint32_t l_screenHeight)
{
    ...
    {
        std::cout << "ERROR::THIEF::FRAMEBUFFER:: Framebuffer is not complete!" << std::endl;
    }
    else
    {
        //std::cout << "SUCCESS::THIEF::FRAMEBUFFER:: Framebuffer complete!" << std::endl;
    }
 ...
 * @param dt 
 */
void ThiefEffect::Draw(float dt)
{
    m_renderUtilities_ptr->UpdateThiefValues();
    /**
    ...
    m_shader_ptr->UseShader();
    m_shader_ptr->SetVec3f("_average", m_renderUtilities_ptr->GetAverageRGB());
